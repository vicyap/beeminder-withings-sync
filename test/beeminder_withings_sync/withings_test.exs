defmodule BeeminderWithingsSync.WithingsTest do
  ...
  describe "withings_user_infos" do
    ...
    test "get_withings_user_info!/1 returns the withings_user_info with given withings_user_id" do
      withings_user_info = withings_user_info_fixture()

      assert Withings.get_withings_user_info!(withings_user_info.withings_user_id) ==
               withings_user_info
    end
    ...
    test "create_withings_user_info/1 with valid data creates a withings_user_info" do
      ...
      assert {:ok, %WithingsUserInfo{} = withings_user_info} =
               Withings.create_withings_user_info(valid_attrs)
    ...
    test "update_withings_user_info/2 with valid data updates the withings_user_info" do
      ...
      update_attrs = %{
        ...
        withings_user_id: "some updated withings_user_id",
        access_token: "some updated access_token",
        refresh_token: "some updated refresh_token",
        expires_at: ~U[2025-01-23 22:33:00Z],
        token_type: "some updated token_type",
        csrf_token: "some updated csrf_token"
      }

      assert {:ok, %WithingsUserInfo{} = withings_user_info} =
               Withings.update_withings_user_info(withings_user_info, update_attrs)
    ...
    test "update_withings_user_info/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               Withings.update_withings_user_info(withings_user_info, @invalid_attrs)

      assert withings_user_info ==
               Withings.get_withings_user_info!(withings_user_info.withings_user_id)
    end
